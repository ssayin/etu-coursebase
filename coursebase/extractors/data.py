...
class CacheData:
    ...
    def write(self) -> None:
        ...
        with open("{0}/{1}".format(CACHE_DIR, CACHE_FILE), "w") as out:
            json.dump({"c": courses}, out, ensure_ascii=False)

    def read(self) -> dict:
        with open("{0}/{1}".format(CACHE_DIR, CACHE_FILE), "r") as f:
...
class ConfigData:
    ...
    def write(self, courses: list):
        with open("{0}/{1}".format(CONFIG_DIR, CONFIG_FILE), "w") as f:
            json.dump({"lookFor": courses}, f, ensure_ascii=False)

    def read(self):
        with open("{0}/{1}".format(CONFIG_DIR, CONFIG_FILE), "r") as f:
